...
print(f"  α={ALPHA}  γ={GAMMA}  ε_decay={EPS_DECAY}")
print("=" * 60)
...
for episode in range(N_EPISODES):
    ...
    # ── Save policy_v1 at episode 500 (mid-training snapshot) ────────
    if (episode + 1) == 500:
...
print(f"policy_v1 → policies/policy_v1_{RUN_ID}.pkl  (episode 500)")
print(f"policy_v2 → policies/policy_v2_explored_{RUN_ID}.pkl  (episode 1000)")
...
print(f"  {'Avg Reward (last 100 eps)':<28} {'N/A':>8} {round(float(np.mean(rewards_log[-100:])),1):>8}")
print("=" * 50)
